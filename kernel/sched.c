...
/* this is the scheduler proper: */
    ...
    /* 3. 切换到next进程运行，找到LDT1，代码段和数据段基址都设置为0x4000000（见fork.c的84行），
     * 表示找到页目录的第16项，页表和页偏移都为0，而页目录16项是指向一个新的页表（在memery.c的258行）
     * 页表中存放的是进程0的第一个页表的前160项（memery.c的267行），也就是0x0,0x1000,0x2000...0xA0000
     * 共160项。也就是说进程0和进程1页表地址虽然不同，但是指向的地址相同。再看此时进程1的TSS中的eip值
     * 是指向fork中调用int 0x80时的后一行语句，此时切换会将eip=tss.eip，页基址相同，eip也相同，程序
     * 跳转到ini 0x80后一句运行*/
	switch_to(next);
}
