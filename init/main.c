 ...
 */
	 ...
	 * fork函数在父进程调用时返回创建的子进程号，进入子进程后，fork返回0，从fork
	 * 后继续执行*/
...
void init(void)
{
    ...
    /* 2. 创建进程2，返回pid=2 */
	if (!(pid=fork())) {
		/* 4. 进程2进入这里，首先关闭标准输入设备文件，然后打开rc文件替换它 */
		close(0);
	...
	while (1) {
		...
		/* 6. 进程1返回4，不进入这里；进程4返回0，进入这里 */
		if (!pid) {                                 // 新的子进程
			/* 7. 进程4关闭所有打开的文件，重新打开标准输入设备，而非rc文件，这使得
			 * shell进程开始执行后不会退出。*/
